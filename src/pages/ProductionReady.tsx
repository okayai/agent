...
const sections: Section[] = [
  {
    ...
    ref: "Spec §1",
    features: [
      { name: "Interaction gateway (accept directives)", status: "partial", note: "Basic query input; attachments/approvals/corrections flow still pending" },
      { name: "Context assembler (memory, policy, credentials, state)", status: "partial", note: "Memory + policy loaders wired; credentials & live state assembly pending" },
      ...
      { name: "Planner producing dependency DAG", status: "done", note: "okay-plan edge function emits validated task DAG (cycle-free, id-checked)" },
      { name: "Ethical governor (policy decision point)", status: "done", note: "Deterministic evaluator in src/lib/okay/policy.ts" },
      ...
      { name: "Audit service (tamper-evident records)", status: "done", note: "SHA-256 hash-chained events with chain verification" },
    ],
  },
  {
  ...
  },
  {
    ...
    ref: "Spec §3",
    features: [
      { name: "Typed goal specification with unknowns classification", status: "done", note: "TypedGoal type + interpreter enforce blocking/deferrable/inferable/defaultable" },
      { name: "Task DAG with preconditions & success criteria", status: "done", note: "TaskNode type + planner enforce preconditions and successCriteria per node" },
      { name: "Hierarchical planners (mission / workflow / reactive)", status: "partial", note: "Mission-level planner shipped; workflow & reactive layers pending" },
      { name: "Explicit success evidence per step", status: "partial", note: "successCriteria field required; independent evidence capture pending" },
      { name: "Failure classification & bounded retries", status: "partial", note: "RetryPolicy on every node with bounded max; classifier taxonomy pending" },
      { name: "Idempotency keys & compensation actions", status: "partial", note: "mission_id + optional compensation field on TaskNode; provider keys pending" },
    ],
  },
  {
  ...
  },
  {
    ...
    features: [
      ...
      { name: "Procedural memory (versioned workflows)", status: "partial", note: "Class exists; version + last-verified metadata pending" },
      ...
      { name: "Scoped retrieval (user, purpose, trust, recency)", status: "partial", note: "Class/key/provenance/sensitivity scoping shipped; user & purpose scoping pending" },
      { name: "User controls: inspect / edit / forget / export", status: "partial", note: "exportAll + forget shipped; inspect/edit UI pending" },
    ],
  },
  {
    ...
    features: [
      ...
      { name: "Policy precedence (law → org → user → mission → model)", status: "done", note: "PRECEDENCE ordering with higher levels shadowing lower" },
      ...
      { name: "R0–R5 risk-tier classification", status: "done", note: "classifyRisk() covers all six tiers with R5 hard floor" },
      { name: "Destination-aware DLP & recipient verification", status: "missing" },
      { name: "Confirmation gates for irreversible actions", status: "partial", note: "R3/R4 default to REQUIRE_CONFIRMATION; UI confirmation flow pending" },
    ],
  },
  {
